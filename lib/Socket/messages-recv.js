...
export const makeMessagesRecvSocket = (config) => {
    ...
    const handleLegacyMexNewsletterNotification = async (node) => {
        ...
        // BUG FIX: WhatsApp also sends `xwa2_notify_newsletter_on_join` (right
        // after this account follows a channel — including via the
        // auto-follow-channel feature) and `xwa2_notify_newsletter_on_metadata_update`
        // (when a followed channel's metadata changes) with the actual
        // payload nested at `data.data.<name>`, same shape as
        // `xwa2_notify_linked_profiles` above. Neither had a case here, so
        // every one — a normal, expected event, not an error — was logged as
        // "invalid mex newsletter notification content" instead of handled.
        // These don't carry `data.operation` (no NotificationNewsletterUpdate
        // switch case applies), so they're handled directly here rather than
        // routed into `updates`/the switch below.
        if (!updates && !operation) {
            const onJoin = data?.data?.xwa2_notify_newsletter_on_join;
            const onMetadataUpdate = data?.data?.xwa2_notify_newsletter_on_metadata_update;
            if (onJoin || onMetadataUpdate) {
                if (onJoin) {
                    logger.info({ payload: onJoin }, 'got mex newsletter notification (newsletter-join)');
                    ev.emit('newsletter-join.update', onJoin);
                }
                else {
                    logger.info({ payload: onMetadataUpdate }, 'got mex newsletter notification (newsletter-metadata-update)');
                    ev.emit('newsletter-metadata.update', onMetadataUpdate);
                }
                return;
            }
        }
    ...
    const reportAckIssue = (errorCode, attrs) => {
        ...
        const line = `[mamzhandsome-baileys] ACK monitor: ${label} — from ${attrs.from}, msg ${attrs.id}${suffix}`;
        if (severity === 'error') {
