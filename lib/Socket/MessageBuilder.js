...
class Toolkit {
  ...
  static async resize(buffer, x, y, fit = "cover") {
    // Lazy-loaded: sharp is a required peerDependency of this package, but
    // loading it only here (not at module top level) means importing this
    // file doesn't fail for anyone who hasn't installed it unless they
    // actually call this specific method.
  ...
  static async getMp4Preview(
    ...
    // Lazy-loaded, same reasoning as Toolkit.resize()'s sharp import above:
    // fluent-ffmpeg isn't a dependency of this package at all (unlike sharp,
    // which is at least a required peerDependency) — loading it only here
    // means the rest of this file, and this whole package, keeps working
    // for anyone who never calls this specific method.
    ...
    const notif = notification
      ? {
          sessionTransparencyMetadata: {
            disclaimerText: "XY-Mamzhandsome",
            hcaId: `hca_${Date.now()}`,
