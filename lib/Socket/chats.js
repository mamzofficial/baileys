...
export const makeChatsSocket = (config) => {
    ...
    const updateBlockStatus = async (jid, action) => {
        ...
        // logger) so it's obvious when this account blocks/unblocks someone.
        const actionLabel = action === 'block' ? 'Blocked' : 'Unblocked';
        console.log(`\x1b[36m[mamzhandsome-baileys] \u{1F6AB}  ${actionLabel}: ${jid}\x1b[0m`);
        config.logger?.info?.({ jid, action }, `${action === 'block' ? 'blocked' : 'unblocked'} user`);
    };
