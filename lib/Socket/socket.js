...
import { addTransactionCapability, aesEncryptCTR, bindWaitForConnectionUpdate, buildPairingQRData, bytesToCrockford, checkWhatsAppBanStatus, configureSuccessfulPairing, Curve, derivePairingCodeKey, generateLoginNode, generateMdTagPrefix, generateRegistrationNode, getCodeFromWSError, getCompanionPlatformId, getErrorCodeFromStreamError, getNextPreKeysNode, makeEventBuffer, makeNoiseHandler, promiseTimeout, signedKeyPair, xmppSignedPreKey } from '../Utils/index.js';
...
export const makeSocket = (config) => {
    ...
    /**
     * Look up a WhatsApp username (e.g. "some.name") via USync.
     * @param {string | string[]} username - username(s) to fetch
     * @param {string} [pin] - the username's pin, if the user has one set
    ...
    /**
     * Check a WhatsApp number's ban/safety status via an unofficial third-party
     * checker API. OPT-IN, not called anywhere automatically. See
     * lib/Utils/check-whatsapp-ban.js for important caveats about this
     * external, unverified service before relying on it.
     * @param {string} number - phone number, digits only (with or without '+')
     */
    const checkBan = async (number) => {
        const result = await checkWhatsAppBanStatus(number);
        return result.raw;
    };
