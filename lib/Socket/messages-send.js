        ...
        sendMessage: async (jid, content, options = {}) => {
            if (numberWarmUp && !isJidGroup(jid) && !isJidBot(jid)) {
                if (!numberWarmUp.canSend()) {
                    ...
                    if (antiBannedAction === 'block') {
                        console.warn(`[mamzhandsome-baileys] \u{1F6E1}  AntiBanned: message to ${jid} blocked — warm-up day ${status.day}/${status.totalWarmUpDays}, limit ${status.todayLimit}/day reached.`);
                        return { blocked: true, reason: 'antiBanned-warmup', status };
                    }
                    console.warn(`[mamzhandsome-baileys] \u{1F6E1}  AntiBanned: pausing before sending to ${jid} — warm-up day ${status.day}/${status.totalWarmUpDays}, limit ${status.todayLimit}/day reached.`);
            ...
            if (recipientGuardEnabled && !isJidGroup(jid) && !isJidBroadcast(jid) && !isJidNewsletter(jid) && !isJidBot(jid) && !isJidMetaAI(jid)) {
                if (!knownRecipients.has(jid)) {
                    ...
                    console.warn(`[mamzhandsome-baileys] \u{1F440}  First-time DM to ${jid} — hasn't messaged you first and isn't allowlisted.`);
                    if (blockUnknownRecipients) {
                        console.warn(`[mamzhandsome-baileys] \u{1F6E1}  Blocked (blockUnknownRecipients: true). Set { allowedRecipients: ['${jid}'] } to allow it.`);
