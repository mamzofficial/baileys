...
(function (XWAPaths) {
    ...
    // --- Added from xbailsync@1.0.7: paths for methods newly wired in
    // newsletter.js (subscriber list, generic newsletter update, block user).
    ...
    // --- Added: broader newsletter/channel admin, directory, and moderation
    // surface not previously mapped. Pure additive data (string constants),
    // doesn't change any existing key's value. See LITERACY.md changelog.
...
(function (QueryIds) {
    ...
    QueryIds["UPDATE_METADATA"] = "25976052865358570"; // corrected against xbailsync@1.0.7 (was a placeholder, unused)
    QueryIds["METADATA"] = "26099060629688646"; // corrected against xbailsync@1.0.7 (was a placeholder, unused; NOT the same as the separate local QueryIds.METADATA in newsletter.js, which is already working and untouched)
    ...
    // --- Corrected against xbailsync@1.0.7: these were placeholder values,
    // not wired into any function anywhere (confirmed via a fork-wide search)
    // — safe to correct with no risk to existing behavior. Now wired into
    // Socket/newsletter.js's newly-added admin/directory/wamo methods.
